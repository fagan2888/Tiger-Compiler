...
fun yyAction54 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (if (Option.valOf (Int.fromString (substring (yytext,1,(size yytext)-1))))>255 then (ErrorMsg.error yypos ("illegal ascii char " ^ yytext)) else (buildString := (!buildString) ^ (Char.toString (Char.chr (Option.valOf (Int.fromString (substring (yytext,1,(size yytext)-1))))))); continue())
      end
        ...
        | SOME(inp, strm') =>
            if inp = #"]"
              ...
              then if inp = #"A"
                  ...
                  then yyQ65(strm', yyMATCH(strm, yyAction0, yyNO_MATCH))
                  else yyQ58(strm', yyMATCH(strm, yyAction0, yyNO_MATCH))
            else if inp = #"a"
              then yyQ58(strm', yyMATCH(strm, yyAction0, yyNO_MATCH))
            else if inp < #"a"
              then if inp = #"_"
...
fun yyQ27 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction29(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"/"
              then yyQ110(strm', yyMATCH(strm, yyAction29, yyNO_MATCH))
