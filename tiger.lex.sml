...
fun eof() =
  let
    val pos = hd(!linePos)
  in
    if !nestedComments>0 then (nestedComments := 0; ErrorMsg.error pos ("reached eof inside comment")) else ();
...
fun yyAction44 (strm, lastMatch : yymatch) = (yystrm := strm;
      (nestedComments := 1; YYBEGIN COMMENT; continue()))
fun yyAction45 (strm, lastMatch : yymatch) = (yystrm := strm;
      (ErrorMsg.error yypos ("end comment with no start comment"); continue()))
fun yyAction46 (strm, lastMatch : yymatch) = (yystrm := strm;
      (nestedComments := !nestedComments+1; continue()))
fun yyAction47 (strm, lastMatch : yymatch) = (yystrm := strm;
      (nestedComments := !nestedComments-1; if (!nestedComments)=0 then YYBEGIN INITIAL else YYBEGIN COMMENT; continue()))
fun yyAction48 (strm, lastMatch : yymatch) = (yystrm := strm;
      (lineNum := !lineNum+1; linePos := yypos :: !linePos; continue()))
fun yyAction49 (strm, lastMatch : yymatch) = (yystrm := strm; (continue()))
fun yyAction50 (strm, lastMatch : yymatch) = (yystrm := strm;
      (insideString := 1; buildString := ""; stringStart := yypos; YYBEGIN STRING; continue()))
fun yyAction51 (strm, lastMatch : yymatch) = (yystrm := strm;
      (insideString := 0; YYBEGIN INITIAL; Tokens.STRING(!buildString, !stringStart, yypos+1)))
fun yyAction52 (strm, lastMatch : yymatch) = (yystrm := strm;
      (buildString := (!buildString) ^ "\n"; lineNum := !lineNum+1; linePos := yypos :: !linePos; continue()))
fun yyAction53 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (buildString := (!buildString) ^ yytext; continue())
      end
fun yyAction54 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (if (Option.valOf (Int.fromString (String.substring (yytext,1,(String.size yytext)-1))))>255 then (ErrorMsg.error yypos ("illegal ascii char " ^ yytext)) else (buildString := (!buildString) ^ (Char.toString (Char.chr (Option.valOf (Int.fromString (String.substring (yytext,1,(String.size yytext)-1))))))); continue())
      end
fun yyAction55 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (buildString := (!buildString) ^ (String.str (Option.valOf (Char.fromString yytext))); continue())
      end
...
fun yyQ27 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction29(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"/"
              then yyQ107(strm', yyMATCH(strm, yyAction29, yyNO_MATCH))
...
fun yyQ16 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction49(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"/"
              then yyQ19(strm', yyMATCH(strm, yyAction49, yyNO_MATCH))
              else yyAction49(strm, yyNO_MATCH)
      (* end case *))
